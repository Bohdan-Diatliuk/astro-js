...
export default function ConfirmCard({ date, formattedDate, lat, lng, locationLabel, mapsLink }: Props) {
    ...
    <div className="grid items-center gap-12 lg:grid-cols-2">
      ...
      <div className="rounded-[28px] bg-card px-8 py-10 text-center shadow-[0_30px_60px_-15px_rgba(61,44,46,0.22)] sm:px-10">
        <div className="flex justify-center gap-2" aria-hidden="true">
          {[0, 1, 2, 3].map((segment) => (
            <span key={segment} className="h-0.5 w-8 rounded-full bg-accent/50" />
          ))}
        </div>
        ...
        <div className="mt-7 grid grid-cols-2 gap-3 text-left">
          <div className="rounded-2xl bg-canvas-start/70 p-4">
            ...
            <p className="mt-3 text-xs font-semibold uppercase tracking-wide text-ink-soft">Дата</p>
          ...
          <div className="rounded-2xl bg-canvas-start/70 p-4">
            ...
            <p className="mt-3 text-xs font-semibold uppercase tracking-wide text-ink-soft">Місце</p>
            <a
              href={mapsLink}
              target="_blank"
              rel="noreferrer"
        ...
        {status === 'error' && <p className="mt-3 text-sm text-red-600">Щось пішло не так, спробуй ще раз.</p>}
