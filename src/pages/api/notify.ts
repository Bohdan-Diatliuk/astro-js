...
export const POST: APIRoute = async ({ request }) => {
	...
	if (!botToken || !chatId) {
		return new Response(JSON.stringify({ error: 'Server is not configured' }), { status: 500 });
	}

	const { date, location } = payload;
	const formattedDate = new Intl.DateTimeFormat('uk-UA', { dateStyle: 'long' }).format(
	...
	if (!telegramResponse.ok) {
		return new Response(JSON.stringify({ error: 'Failed to notify' }), { status: 502 });
	}
